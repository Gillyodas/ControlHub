...
import { Plus, Edit, Trash2, Eye, EyeOff, Save, Power } from "lucide-react"
import { getIdentifierConfigs, createIdentifierConfig, toggleIdentifierActive, updateIdentifierConfig, ValidationRuleType, type IdentifierConfigDto, type CreateIdentifierConfigCommand, type ValidationRuleDto } from "@/services/api/identifiers"
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="grid gap-4">
            {configs.map((config) => (
              <IdentifierConfigCard key={config.id} config={config} />
            ))}
          </div>
        )}
          ...
          <div className="space-y-2">
            {config.rules.map((rule, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-gray-700 rounded-lg">
                <div className="flex-1">
                  <div className="font-medium text-sm text-gray-100">{rule.type}</div>
